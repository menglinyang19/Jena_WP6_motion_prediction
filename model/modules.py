...
class MLP(nn.Module):
    def __init__(self, config):     # initialize the MLP and take a configuration dictionary 'config' as input
        super().__init__()
        self._config = config
        modules = []            # initialize an empty list to store the layers of the network
        assert config["n_layers"] > 0       # assert the number is greater than zero
        for i in range(config["n_layers"]):
            # For each layer, a fully connected layer is added to the list of modules
            modules.append(nn.Linear(config["n_in"], config["n_out"]))
            if i < config["n_layers"] - 1:
                # not the last layer: batch normalization and/or dropout layers may be added
                if config["batchnorm"]:
                    modules.append(nn.BatchNorm1d(config["n_out"]))
                if config["dropout"]:
                    modules.append(nn.Dropout(p=0.1))
                modules.append(nn.ReLU())
        self._mlp = nn.Sequential(*modules)         # construct a new neural network with layers listed in modules
        self.n_in = config["n_in"]
        self.n_out = config["n_out"]
    
    def forward(self, x):
        # The input tensor is passed through the _mlp object, which applies all of the layers in sequence.
...
class NormalMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        try:
            ...
            # Return a default tensor or value to continue training
            return torch.zeros_like(x)  # Return a tensor with zeros
        tmp.append(x)
        for l in self._mlp:
            try:
                x = l(x)
...
class CGBlock(nn.Module):           # a context gating block
    def __init__(self, config):
        super().__init__()
        self._config = config
        # build MLP networks for the 'sentence' and 'context' input
    ...
    def forward(self, scatter_numbers, s, c):
        ...
        s = self.s_mlp(s.view(-1, s.shape[-1])).view(prev_s_shape)  # view(): reshape input tensors for processing
...
class MCGBlock(nn.Module):      # multi-layer CGBlock
    def __init__(self, config):
        super().__init__()
        self._config = config
        self._blocks = []
    ...
    def forward(
        ...
        if c is None:
            ...
            #c = torch.ones(s.shape[0], 1, self.n_in, requires_grad=True).cpu()
        else:
...
class EM(nn.Module):
    # input: 1. the non-normalized log-probabilities of each data point belonging to each of the mixture components
    #        2. the observed trajectories (i.e., sequences of data points)
    #        3. the covariance matrices of the mixture components
    # output 1. the probabilities of each trajectory belonging to each of the final mixture components
    #        2. the estimated means of each of the final mixture components
    #        3. the estimated covariance matrices of each of the final mixture components
    def __init__(self):
        super().__init__()
        # selected 3 trajectories to initialize the GMM; furthest apart from each other
        self._selector = torch.LongTensor([29, 49, 79])
        self._n_final_trajs = 6     # desired number of initial trajectories
    
    @torch.no_grad()
    #  computes the initial assignments of each trajectory to one of the final mixture components,
    #  based on the distances between the trajectories and a set of predefined anchor points
    def _compute_initial_state(self, probas, trajectories):
        # probas -- the probability of each cluster for each trajectory
        # trajectories -- the atomic positions of each trajectory
        ...
        #result_idx = torch.zeros((trajectories.shape[0], self._n_final_trajs), dtype=torch.long).cpu()
        #worked = torch.ones((trajectories.shape[0], trajectories.shape[1])).cpu()
        ...
        # Trajectories that are closer to each other than this threshold (0.15) are considered to be in the same cluster
        ...
        # select 6 trajectories with the highest scores
    ...
    def _compute_coefficients(self, _covariance_matrices6, trajectories, trajectories6, probas6):
        # computes the coefficients of the Gaussian distributions that approximate the likelihood of each trajectory
        # belonging to each mixture component, given the observed data
        covariance_matrices6 = _covariance_matrices6# * 500.0
        # matrics of the 6 Gaussian distributions represented by the 6 trajectories selected above
        ...
        qform = (A @ C @ B)[..., 0, 0]      # the quadratic form of the Mahalanobis distance
    ...
    def forward(self, non_normalized_probas, trajectories, covariance_matrices):
        # iterative EM algorithm, repeatedly estimating the coefficients and probabilities
...
class MHA(nn.Module):   # Multi-head Attention
    ...
    def forward(self, traj_embeddings):
        ...
        #target_scatter_numbers = torch.ones(batch_size, dtype=torch.long).cpu()
        #target_scatter_idx = torch.arange(batch_size, dtype=torch.long).cpu()
