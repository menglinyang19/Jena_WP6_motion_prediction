...
import random
import torch
from torch.utils.data import Dataset, DataLoader   # generator check

# convert a yaw angle to a range between -pi and pi
def angle_to_range(yaw):
    yaw = (yaw - np.pi) % (2 * np.pi) - np.pi
    return yaw

# normalize features from the configuration dictionary
# the following values should be replaced by the actual data
def normalize(data, config):
    ...
    if features == ("xy", "yaw", "speed", "valid"):
        normalizarion_means = {
            ...
            "road_network_embeddings": np.array(
                [8.75743078e+001, -1.27234378e-001, 2.67789801e-001, -1.49505650e-002, 4.02657944e-002, 4.00775670e+000,
                 2.12165843e+000, 0.00000000e+000, 0.00000000e+000, 3.90854185e-001, 0.00000000e+000, 0.00000000e+000,
                 0.00000000e+000, 1.32873167e-001, 2.54529767e-002, -1.73911107e-321, 1.72562554e-003, 0.00000000e+000,
                 6.03968939e-002, 6.47109577e-003, 0.00000000e+000, 0.00000000e+000, 1.63503020e-001, 8.36928387e-002,
                 2.58843831e-003, 3.27868852e-002, 2.11389129e-002, 7.85159620e-002],
                dtype=np.float32)
        ...
        normalizarion_stds = {
            ...
            "road_network_embeddings": np.array([37.5536320e+00, 7.66946847e-01, 6.28489375e-01, 7.47651862e-01,
                                                6.62275398e-01, 1.09563440e+00, 2.13764233e+00, 7.26703318e-02,
                                                9.52921034e-02, 4.91450947e-01, 1.87818907e-01, 0.00000000e+00,
                                                0.00000000e+00, 2.73591130e-01, 2.50444992e-01, 7.87196305e-03,
                                                4.74474343e-02, 1.21130896e-02, 1.05682025e-01, 1.28477277e-01,
                                                8.18965510e-02, 0.00000000e+00, 3.98579035e-01, 1.82350700e-01,
                                                3.66290809e-02, 1.79525920e-01, 1.14256943e-01, 2.61386473e-01
                                                ], dtype=np.float32)
        }
        ...
        normalizarion_stds = {
            ...
            "road_network_embeddings": np.array([37.5536320e+00, 7.66946847e-01, 6.28489375e-01, 7.47651862e-01,
                                                ...
                                                4.74474343e-02, 1.21130896e-02, 1.05682025e-01, 1.28477277e-01,
                                                8.18965510e-02, 0.00000000e+00, 3.98579035e-01, 1.82350700e-01,
                                                3.66290809e-02, 1.79525920e-01, 1.14256943e-01, 2.61386473e-01], dtype=np.float32)
        }
    ...
    for k in keys:
        ...
        # plus a small value 1e-6 to avoid division by zero
        data[k].clamp_(-15, 15)         # normalize data to a range [-15,15]
...
def dict_to_cuda(d):
#def dict_to_cpu(d):
    passing_keys = {'target/history/lstm_data', 'target/history/lstm_data_diff', 'other/history/lstm_data',
                    'other/history/lstm_data_diff', 'target/history/mcg_input_data', 'other/history/mcg_input_data',
                    'other_agent_history_scatter_idx', 'road_network_scatter_idx',
                    'other_agent_history_scatter_numbers', 'road_network_scatter_numbers', 'batch_size',
                    'road_network_embeddings', 'target/future/xy', 'target/future/valid'}
    for k in d.keys():
...
class MultiPathPPDataset(Dataset):
    ...
    # computer difference features between consecutive time steps in the data
    ...
    # compute one-hot encoded vectors of the agent type and vehicle status
    ...
    # randomly mask part of the historical data for agents
    def _mask_history(self, ndarray, fraction):
        assert fraction >=0 and fraction < 1
        ndarray = ndarray * (np.random.uniform(size=ndarray.shape) > fraction)
        return ndarray

    # stack historical data and convert it to the format expected by the LSTMs in the model
    ...
    # override the default method in Dataset.py
    # load data and convert part of the data into a different format
    def __getitem__(self, idx):
        ...
        # Extract the desired columns (0 to 27) from 'road_network_embeddings'
        np_data['road_network_embeddings'] = np_data['road_network_embeddings'][:, :, 0:28]
        
        return np_data

    @staticmethod
    # process and combine individual samples in a batch-like manner
...
def get_dataloader(config):
    ...
    # DataLoader object is used to iterate over the dataset in batches during the training
