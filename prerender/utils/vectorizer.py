...
class MultiPathPPRenderer(Renderer):
    def __init__(self, config):
        self._config = config
        # NOTE:pay attention to the number set here, according to the database, it should be [0,19]
    ...
    def _prepare_roadnetwork_info(self, data):
        ...
        node_type = data["roadgraph_samples/type"].flatten()
        result = []
        segment_types = []
        ...
        for polyline_id in np.unique(node_id):
            ...
            if "drop_segments" in self._config:
                # selector -- index for nodes that will be reserved
        ...
        assert len(segment_types) == len(result), \
            f"Number of segments {len(result)} doesn't match the number of types {len(segment_types)}"
        return {
            "segments": result,
    ...
    def _transform_to_agent_coordinate_system(self, coordinates, shift, yaw):
        # coordinates
    ...
    def _generate_segment_embeddings(self, segments, types):
        ...
        # NOTE: this is the temporary treatment
    ...
    def render(self, data):
        ...
        for i in range(agent_history_info["history/xy"].shape[0]):
            ...
            # transform the coordinates of the road network segments to the agent's coordinate system
            current_scene_road_network_coordinates = self._transform_to_agent_coordinate_system(
                road_network_info["segments"], current_agent_scene_shift, current_agent_scene_yaw)
            # filter the transformed coordinates and segment types based on certain criteria
            ...
            road_segments_embeddings = self._generate_segment_embeddings(
                current_scene_road_network_coordinates, current_scene_road_network_types)
            current_scene_agents_coordinates_history = self._transform_to_agent_coordinate_system(
                agent_history_info["history/xy"], current_agent_scene_shift,
                current_agent_scene_yaw)
            current_scene_agents_coordinates_future = self._transform_to_agent_coordinate_system(
            ...
            scene_data = {
                ...
                "scenario_id": agent_history_info["scenario_id"].item(),
